...
class SerializeAllDataClasses {
	...
	void testLoadInstance(Class<?> persistentClass, Class<?> dataClass)
			throws ReflectiveOperationException, IllegalArgumentException, SecurityException, IOException {
	...
	static Stream<Arguments> classList() {
		Reflections reflections = new Reflections(
				new ConfigurationBuilder().forPackages("de.kreth.clubhelper.entity").addScanners(Scanners.SubTypes));

		Set<Class<? extends Serializable>> persistentClasses = reflections.getSubTypesOf(Serializable.class);

		reflections = new Reflections(
				new ConfigurationBuilder().forPackages("de.kreth.clubhelper.data").addScanners(Scanners.SubTypes));
		Set<Class<? extends Serializable>> dataClasses = reflections.getSubTypesOf(Serializable.class);

		return persistentClasses.stream().filter(c -> !Modifier.isAbstract(c.getModifiers())).filter(c -> !c.isEnum())
				.filter(c -> !c.isMemberClass()).map(c -> map(c, dataClasses));
