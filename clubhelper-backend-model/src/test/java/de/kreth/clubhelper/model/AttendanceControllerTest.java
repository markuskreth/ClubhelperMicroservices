...
//	attendance.setCreated(created);
	...
	void callAttendanceListEmpty() throws Exception {
		mvc.perform(get("/attendance/" + onDate.format(DateTimeFormatter.ISO_DATE))).andExpect(status().isOk())
	...
	void callAttendanceListOne() throws Exception {
		...
		mvc.perform(get("/attendance/" + onDate.format(DateTimeFormatter.ISO_DATE))).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON.toString()))
				.andExpect(content()
						.json("[{\"id\":1,\"changed\":null,\"created\":null,\"deleted\":null,\"onDate\":\"2020-10-10\""
